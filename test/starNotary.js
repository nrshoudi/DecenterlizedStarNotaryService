...
contract('StarNotary', async (accs) => {
  ...
  it('2 users can exchange their stars.', async() => {
    ...
    await instance.createStar('first star', starId_1, {from: user1})
    await instance.createStar('second star', starId_2, {from: user2})

    await instance.approve(user1, starId_2, {from: user2})

    await instance.exchangeStars(starId_1, starId_2, {from: user1})

    assert.equal(await instance.ownerOf.call(starId_2), user1)
    assert.equal(await instance.ownerOf.call(starId_1), user2)
